...
if __name__ == "__main__":
    ...
    if args.plot:
        # 音声の長さに応じて動的に幅を調整
        duration_minutes = duration / 60  # 分単位の長さ
        
        # 基本幅は12、1分につき4インチ追加（最大40インチまで）
        width = min(40, max(12, 12 + duration_minutes * 4))
        height = 8  # 高さは固定
        
        print(f"Audio duration: {duration}s ({duration_minutes:.1f}min)")
        print(f"Figure size: {width:.1f} x {height}")
        ...
        fig, ax = plot_stereo(
            ...
            vad=out["vad"][0].cpu(),
            figsize=(width, height)  # 動的に計算したサイズを指定
        )
