...
def extract_preds_and_targets(
    ...
    def get_targets(labels):
        ...
        targets = []
        for lab in labels:
            targets.append(1 if lab == "pred_shift" else 0)
        return targets
...
def plot_accuracy_bc_vs_predbc(af, N=None, figsize=(8, 6)):
    ...
    if N is not None:
        fig.suptitle(
            f"{N['total']} Events. pred_shift: ({100*N['pred_shift']:.1f}%) pred_hold: ({100*N['pred_hold']:.1f}%)"
        )
    ax = plt.subplot()
...
def calculate_accuracy(df):
    ...
    for t in thresholds:
        ...
        for name in pred_names:
            ...
            pred_shift_acc = correct[targets == 1].mean().item()
            pred_hold_acc = correct[targets == 0].mean().item()
            row[f"acc_{name}"] = correct.mean().item()
            row[f"bacc_{name}"] = (pred_shift_acc + pred_hold_acc) / 2
            row[f"acc_pred_shift_{name}"] = pred_shift_acc
            row[f"acc_pred_hold_{name}"] = pred_hold_acc
...
def simple_label_stats(df: pd.DataFrame):
    n = len(df)
    n_pred_shift = len(df[df["target"] == 1])
    n_pred_hold = len(df[df["target"] == 0])
    return {"total": n, "pred_shift": n_pred_shift / n, "pred_hold": n_pred_hold / n}
...
def evaluation(args):
    ...
    metadata["events_pred_shift"] = label_stats["pred_shift"]
    metadata["events_pred_hold"] = label_stats["pred_hold"]
