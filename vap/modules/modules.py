...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x):
        # Check if the tensor is in [B, D, L] format
        if len(x.shape) == 3 and x.size(1) == self.inp_dim:
            # Transpose, apply layer norm, project, then transpose back
            x_transposed = x.transpose(1, 2)  # [B, L, D]
            x_norm = self.layer_norm(x_transposed)
            x_proj = self.projection(x_norm)
            x_out = x_proj.transpose(1, 2)  # [B, D', L]
            return self.dropout(x_out)
        else:
            # Standard processing for [B, L, D] or other formats
            x = self.layer_norm(x)
            x = self.projection(x)
            return self.dropout(x)
