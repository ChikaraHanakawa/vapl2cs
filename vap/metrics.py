...
class VAPMetric:
    ...
    def extract_prediction_and_targets(
        ...
        for b in range(batch_size):
            ...
            ###########################################
            # Process backchannel prediction data - positive samples
            if "pred_backchannel" in events and b < len(events["pred_backchannel"]):
                for start, end, speaker in events["pred_backchannel"][b]:
                    # Probability of backchannel
                    pbc = p_fut[b, start:end]
                    if speaker:
                        pbc = 1 - pbc
                    preds["bp"].append(pbc)
                    # Positive samples are ones (predicting backchannel)
                    targets["bp"].append(torch.ones_like(pbc))
                    
            # Process backchannel prediction data - negative samples
            if "pred_backchannel_neg" in events and b < len(events["pred_backchannel_neg"]):
                for start, end, speaker in events["pred_backchannel_neg"][b]:
                    # Probability of no backchannel
                    pbc_neg = 1 - p_fut[b, start:end]
                    if speaker:
                        pbc_neg = 1 - pbc_neg
                    preds["bp"].append(pbc_neg)
                    # Negative samples are zeros (no backchannel)
                    targets["bp"].append(torch.zeros_like(pbc_neg))
