...
def extract_preds_and_targets(
    ...
    def get_targets(labels):
        ...
        targets = []
        for lab in labels:
            targets.append(1 if lab == "pred_backchannel" else 0)
        return targets
...
def plot_accuracy_bc_vs_predbc(af, N=None, figsize=(8, 6)):
    ...
    if N is not None:
        fig.suptitle(
            f"{N['total']} Events. pred_backchannel: ({100*N['pred_backchannel']:.1f}%) pred_backchannel_neg: ({100*N['pred_backchannel_neg']:.1f}%)"
        )
    ax = plt.subplot()
...
def calculate_accuracy(df):
    ...
    for t in thresholds:
        ...
        for name in pred_names:
            ...
            pred_backchannel_acc = correct[targets == 1].mean().item()
            pred_backchannel_neg_acc = correct[targets == 0].mean().item()
            row[f"acc_{name}"] = correct.mean().item()
            row[f"bacc_{name}"] = (pred_backchannel_acc + pred_backchannel_neg_acc) / 2
            row[f"acc_pred_backchannel_{name}"] = pred_backchannel_acc
            row[f"acc_pred_backchannel_neg_{name}"] = pred_backchannel_neg_acc
...
def simple_label_stats(df: pd.DataFrame):
    n = len(df)
    n_pred_backchannel = len(df[df["target"] == 1])
    n_pred_backchannel_neg = len(df[df["target"] == 0])
    return {"total": n, "pred_backchannel": n_pred_backchannel / n, "pred_backchannel_neg": n_pred_backchannel_neg / n}
...
def evaluation(args):
    ...
    fig, ax = plot_accuracy_bc_vs_predbc(af, N=label_stats)

    metadata = vars(args)
    metadata["events_total"] = label_stats["total"]
    metadata["events_pred_backchannel"] = label_stats["pred_backchannel"]
    metadata["events_pred_backchannel_neg"] = label_stats["pred_backchannel_neg"]
